...
def plot_avg_sentiment_per_game(df):
    """
    Calculates and plots the average sentiment score per game.
    """
    avg_sentiment_per_game = df.groupby('game_name')['sentiment_score'].mean().sort_values()
    plt.figure(figsize=(12, 8))
    barplot = sns.barplot(x=avg_sentiment_per_game.values, y=avg_sentiment_per_game.index, palette="RdGy", edgecolor="w")
